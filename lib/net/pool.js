...
class Pool extends EventEmitter {
  ...
  setLoader(peer) {
    ...
    assert(peer.outbound || this.options.incoming_sync);
    assert(!this.peers.load);
    assert(!peer.loader);
  ...
  async handleOpen(peer) {
    ...
    if (peer.outbound || this.options.incoming_sync){
      this.sendSync(peer);
  ...
  async handleGetBlocks(peer, packet){
    ...
    if (this.chain.options.prune)
      return;
    if (!this.chain.synced && !this.options.assist_before_sync)
      return peer.send(new packets.InvPacket([]));
...
class PoolOptions {
  ...
  constructor(options) {
    ...
    this.upnp = false;
    this.selfish = false;
    this.incoming_sync = false;
    this.assist_before_sync = false;
  ...
  fromOptions(options) {
    ...
    if (options.incoming_sync) {
      assert(typeof options.incoming_sync === 'boolean');
      this.incoming_sync = options.incoming_sync;
    }

    if (options.assist_before_sync) {
      assert(typeof options.assist_before_sync === 'boolean');
      this.assist_before_sync = options.assist_before_sync;
    }
