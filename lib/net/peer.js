...
const {ewait} = require('../utils/util');
...
class Peer extends EventEmitter {
  ...
  init() {
    this.parser.on('packet', async (packet) => {
      try {
        ...
        if (packet.type==packetTypes.VERSION)
  ...
  async initConnect() {
    ...
    this.connectTimeout = setTimeout(() => {
      this.connectTimeout = null;
      cleanup();
      wait.throw(new Error('Connection timed out.'));
    }, 10000);
    return await wait;
  }
  ...
  destroy() {
    ...
    if (connected)
      this.logger.info('Closing connection %s.', this.hostname());
  ...
  addTimeout(packet) {
    ...
    switch (packet.type) {
      case packetTypes.MEMPOOL:
        D && console.log('addTimeout MEMPOOL INV');
        this.request(packetTypes.INV, timeout);
        break;
      case packetTypes.GETBLOCKS:
        D && console.log('addTimeout GETBLOCKS INV');
  ...
  maybeTimeout() {
    ...
    for (const [key, entry] of this.responseMap) {
      if (now > entry.timeout) {
        ...
        D && console.log('STALL '+name+' '+key, entry);
        this.destroy();
        return;
      }
    }
  ...
  request(type, timeout) {
    ...
    D && console.log('request '+type, entry);
  ...
  response(type, payload) {
    ...
    D && console.log('response', type, entry);
