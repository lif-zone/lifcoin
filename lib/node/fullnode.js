...
const lif_kv_idx = require('../indexer/lif_kv_idx');
...
class FullNode extends Node {
  ...
  constructor(options){
    ...
    this.fees.init();
    // lif_kv
    if (this.config.bool('lif_kv_idx')) {
      this.lif_kv_idx = new lif_kv_idx({
        network: this.network,
        logger: this.logger,
        blocks: this.blocks,
        chain: this.chain,
    ...
    this.mempool = new Mempool({
      ...
      indexAddress: this.config.bool('index-address'),
      indexAddrSH: this.config.bool('index-addrsh'),
      lif_kv_idx: this.lif_kv_idx,
    });
    // Indexers
    ...
    this.pool = new Pool({
      ...
      incoming_sync: this.config.bool('incoming_sync'),
    });
  ...
  init(){
    ...
    if (this.lif_kv_idx)
      this.lif_kv_idx.on('error', err=>this.error(err));
  ...
  async open(opt){
    ...
    this.opened = true;
    let _opt = opt?.addr_rescan ? {rescan: true} : null;
    await this.handlePreopen();
    await this.blocks.open();
    await this.chain.open();
    if (this.lif_kv_idx)
      await this.lif_kv_idx.open(_opt);
    ...
    if (this.addrindex)
      await this.addrindex.open(_opt);
    if (this.filterIndexers){
      for (const filterindex of this.filterIndexers.values())
  ...
  async startSync(){
    if (this.txindex)
      await this.txindex.sync();
    if (this.addrindex)
      await this.addrindex.sync();
    if (this.lif_kv_idx)
      await this.lif_kv_idx.sync();
    if (this.filterIndexers) {
      for (const filterindex of this.filterIndexers.values()) {
        await filterindex.sync();
      }
    }
    return await this.pool.startSync();
  }
   ...
   */
  async getMetaByAddrSH(addr, options = {}){
    ...
    const confirmed = async ()=>{
      const hashes = await this.addrindex.getHashesByAddrSH(
    ...
    const unconfirmed = ()=>{
      const mempool = this.mempool.getMetaByAddrSH(
        addr, {limit, reverse, after});
  ...
  async lif_kv_get(key){
    if (!this.lif_kv_idx)
      return;
    let kv = await this.lif_kv_idx.get(key);
    if (kv)
      return kv;
    kv = await this.mempool.lif_kv_idx.get(key);
    if (kv)
      return kv;
  ...
  async lif_kv_get_vals(key){
    ...
    let kv = await this.lif_kv_idx.get(key);
    if (kv)
      kvs.push({...kv, confirmed: true});
    kv = await this.mempool.lif_kv_idx.get_vals(key);
    if (kv)
      kvs.push(...kv);
    if (!kvs.length)
      return {not_found: true};
  ...
  async lif_kv_tx_get(tx_hash){
    if (!this.lif_kv_idx)
      return {confirmed: undefined, unconfirmed: undefined};
    if (this.mempool.getTX(tx_hash))
      return await this.mempool.lif_kv_idx.tx_get(tx_hash);
    return await this.lif_kv_idx.tx_get(tx_hash);
  }
  ...
  async getTXByAddress(addr, options = {}){
    const mtxs = await this.getMetaByAddrSH(addr, options);
    const out = [];
