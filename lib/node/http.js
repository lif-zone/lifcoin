...
class HTTP extends Server {
  ...
  async mine_attempt_get({addr, addr_self, nonce1=0, nonce2=0}){
    ...
    if (!mo.time ||
      ...
      mo.time<=time-this.mine_assmble_refresh))
    {
      mo.mempool_seq_last = mo.mempool_seq;
      mo.time = time; // also in attempt.time
    ...
    const reward_sat = Math.floor(reward_share*block_reward);
    const addrs = [{address: addr, value: reward_sat}, addr_self];
    attempt.setAddresses(addrs);
    const coinbase_raw = attempt.getRawCoinbase(nonce1, nonce2);
    const header = attempt.getHeader(
      attempt.getRoot(nonce1, nonce2), attempt.time, 0);
    let offer = {
      nonce1,
      nonce2,
      time: attempt.time,
